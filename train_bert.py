...
# Load Data
data = pd.read_excel('social_media_engagement_data.xlsx', nrows=5000)
...
def extract_bert_features(data_loader, model, device):
    model.eval()
    features = []
...
bert_features = extract_bert_features(data_loader, bert_model, device)

# Combine BERT Features with Other Data
...
X = np.hstack([bert_features, non_text_features.values])
...
# Save Model
joblib.dump(clf, 'engagement_model_bert.pkl')
# Evaluate Model
...
# Save Model
joblib.dump(regressor, 'engagement_model_multioutput.pkl')
